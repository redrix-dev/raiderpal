  ...
  return (
    <div className="relative">
      <button
        ...
        ref={triggerRef}
        className={cn(
          "w-full h-10 2xl:[.ui-compact_&]:h-14 rounded-md border px-3 py-2 text-base 2xl:[.ui-compact_&]:text-sm text-primary flex items-center justify-between focus:outline-none focus:ring-1 focus:ring-brand-cyan hover:border-brand-cyan",
          selected
      ...
      >
        <span className="flex items-center gap-2 truncate">
          ...
          <span
            className={cn("truncate", selected ? "text-primary" : "text-muted")}
          >
      ...
      {open && (
        <div
          className={cn(
            // IMPORTANT: overlay surface must be painted here...
        ...
        >
          ...
          {/* ...and ALSO painted on the scroll wrapper to avoid “transparent seams” */}
          <div className="max-h-80 overflow-y-auto bg-surface-panel">
            {filtered.length === 0 ? (
              <div className="p-3 text-xs text-muted bg-surface-panel">
                No items match your filters.
              </div>
            ) : (
              // ...and painted on the UL as well (belt + suspenders)
              <ul className="text-xs 2xl:[.ui-compact_&]:text-[11px] bg-surface-panel">
                {filtered.map((item) => (
                  <li key={item.id} className="px-2 py-1 bg-transparent">
                    <button
                      type="button"
                      ...
                      className={cn(
                        "group w-full flex items-center gap-3 rounded-lg border px-3 py-2.5 text-left text-sm 2xl:[.ui-compact_&]:text-sm transition-colors hover:border-brand-cyan/70 hover:bg-white/5",
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {(item.itemType || item.lootArea || item.subtitle) && (
                          <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] 2xl:[.ui-compact_&]:text-[10px] text-muted font-medium">
                            {item.itemType && (
